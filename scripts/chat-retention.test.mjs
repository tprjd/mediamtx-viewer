...
it('purges expired messages, removed content, notes, and queued content while keeping structured records', () => {
  ...
  db.prepare('INSERT INTO chat_room (id, channel_id, next_sequence, created_at) VALUES (?, ?, ?, ?)').run(
    'room',
    'channel',
    4,
    expired,
  ...
  db.prepare(
    `INSERT INTO chat_outbox VALUES ('event', 'old', 'chat:channel', '{"type":"message","content":"expired text"}', 0, 0, ?)`,
  ).run(expired)
  db.prepare(
  ...
  db.prepare(`INSERT INTO chat_outbox VALUES
    ('clear', NULL, 'chat:channel', '{"type":"history-cleared","clearedThrough":2}', 0, 0, ?)`)
    .run(expired)
  purgeExpiredChat(db, now)
  expect(db.prepare('SELECT id FROM chat_message').all()).toEqual([
    { id: 'retained' },
  ])
  expect(db.prepare('SELECT id FROM chat_outbox').all()).toEqual([{ id: 'clear' }])
  expect(
    db
      .prepare(
