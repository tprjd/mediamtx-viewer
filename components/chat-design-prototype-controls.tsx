...
import { Ban, Check, Crown, Settings2, ShieldCheck, Timer, X } from 'lucide-react'
  ...
  return <DropdownMenu.Root>
    ...
    <DropdownMenu.Portal><DropdownMenu.Content className={`${styles.menu} ${styles.settingsMenu}`} sideOffset={8} align="end" collisionPadding={12}>
      ...
      <DropdownMenu.CheckboxItem checked={timestamps} onCheckedChange={onTimestampsChange} onSelect={(event) => event.preventDefault()}>
        ...
        <span className={styles.checkBox}><DropdownMenu.ItemIndicator><Check size={13} /></DropdownMenu.ItemIndicator></span>
...
export function PrototypeModerationDialog({ target, onClose, onConfirm }: {
  ...
  return <Dialog.Root open onOpenChange={(open) => { if (!open) onClose() }}>
    <Dialog.Portal><Dialog.Overlay className={styles.dialogOverlay} /><Dialog.Content className={styles.moderationDialog}
      ...
      <div className={styles.dialogHeading}><span className={styles.eyebrow}>CHAT MODERATION</span><Dialog.Close asChild><button aria-label="Close moderation dialog"><X size={18} /></button></Dialog.Close></div>
      ...
      {target.action === 'inspect' ? <p className={styles.dialogHint}>Retained sample content. No action will be applied.</p> : <form onSubmit={(event) => { event.preventDefault(); onConfirm({ category, note, duration }) }}>
        ...
        <label>Private note <span>{category === 'Other' ? 'Required for Other' : 'Optional'}</span><textarea aria-label="Private note" value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} required={category === 'Other'} placeholder="Context for other moderators" /></label>
...
export function PrototypeRestrictionsDialog({ restrictions, onLift }: {
  ...
  return <Dialog.Root>
    ...
    <Dialog.Portal><Dialog.Overlay className={styles.dialogOverlay} /><Dialog.Content className={styles.moderationDialog}>
      <div className={styles.dialogHeading}><span className={styles.eyebrow}>CHAT MODERATION</span><Dialog.Close asChild><button aria-label="Close restrictions"><X size={18} /></button></Dialog.Close></div>
      ...
      <div className={styles.restrictionList}>
        ...
        {restrictions.map((restriction) => <div className={styles.restrictionItem} key={restriction.tag}>
          <div className={styles.restrictionIdentity}>{restriction.action === 'ban' ? <Ban size={17} /> : <Timer size={17} />}<strong>{restriction.name} <span>#{restriction.tag}</span></strong></div>
          ...
          {restriction.note && <small>{restriction.note}</small>}
